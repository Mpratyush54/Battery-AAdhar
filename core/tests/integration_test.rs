...
async fn test_e2e_battery_registration_and_compliance() {
    ...
    let db_pool = sqlx::PgPool::connect_lazy(&db_url).expect("Failed to create dummy pool");
    ...
    let engine =
        BpaEngine::new(db_pool, encryption, jwt_secret, &root_key).expect("engine creation failed");
    ...
    let (_private_key, _public_key) = bpa_engine::services::SigningServiceImpl::generate_keypair()
    ...
    let (_test_seed, _test_pubkey) =
        bpa_engine::services::SigningServiceImpl::generate_keypair().expect("test keypair failed");
    println!("✓ BPAN signed by manufacturer");

    // 5. Create DEK for battery
    let _wrapped_dek = engine
        .key_manager
    ...
    let soh = 87u64;
    let (proof, commitment, _blinding) = engine
        .zk_prover
    ...
    let verify_result = engine.zk_prover.verify_range(&proof, &commitment, 80, 100);
